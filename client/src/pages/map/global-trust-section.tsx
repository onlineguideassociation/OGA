...
import {
  Globe, Shield, Network, Zap, Users, CheckCircle, ArrowRight,
  Lock, Eye, Heart, Code, Activity, Server, Database, Cpu,
  Copy, Play, RefreshCw, ChevronRight, ShieldCheck, Sparkles
} from "lucide-react";
...
export default function GlobalTrustSection() {
  ...
  const filteredEndpoints = activeApiTab === "all"
    ? API_ENDPOINTS
    : API_ENDPOINTS.filter(e => e.status === activeApiTab);
    ...
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-[#0081C9]/20 to-slate-900 p-8 border border-slate-700">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-3xl font-bold text-white mb-2" data-testid="text-trust-title">Global Trust Network</h1>
          <p className="text-lg text-slate-300 mb-1">Connecting Cultures with Loyalty and Truth</p>
          <p className="text-sm text-slate-400 max-w-2xl">
            The OGA (Online Guide Association) Global Trust Network is the unified digital infrastructure
            connecting verified tourism professionals, cultural institutions, and travelers across Southeast Asia
            through a single, transparent API.
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardHeader className="pb-3 border-b">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Network className="h-5 w-5 text-[#0081C9]" />
                  <CardTitle className="text-lg">Southeast Asia Network</CardTitle>
                </div>
                  ...
                  <div key={i} className="flex items-center justify-between p-4 hover:bg-slate-50 transition">
                    <div className="flex items-center gap-3">
                      <div className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold ${
                        ...
                        node.status === "expanding" ? "bg-blue-100 text-blue-700" :
                        node.status === "pilot" ? "bg-amber-100 text-amber-700" :
        ...
        <Card className="border-none shadow-sm">
          <CardHeader className="pb-3 border-b">
            <CardTitle className="text-lg flex items-center gap-2"><Activity className="h-5 w-5 text-emerald-500" /> Network Health</CardTitle>
          </CardHeader>
          <CardContent className="p-4 space-y-4">
            ...
            <div className="pt-3 border-t border-slate-100 space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-slate-500">Total API Calls (24h)</span>
                <span className="font-bold">{trustStats?.apiCalls24h || "24,580"}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-500">Active Connections</span>
                <span className="font-bold">{trustStats?.activeConnections || 213}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-500">Data Centers</span>
                <span className="font-bold">3 (SGP, BKK, PNH)</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-[#0081C9] to-[#C1121F] flex items-center justify-center">
            ...
            {([
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                  activeApiTab === tab.key ? "bg-[#0081C9] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                }`}
                data-testid={`api-filter-${tab.key}`}
              >
                {tab.label}
              </button>
            ))}
        ...
        <Card className="border-none shadow-sm overflow-hidden">
          <div className="bg-slate-900 px-4 py-3 border-b border-slate-700 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Server className="h-4 w-4 text-[#0081C9]" />
              <span className="text-xs font-mono text-slate-300">BASE URL: <span className="text-[#0081C9]">https://onlineguide.io</span></span>
            </div>
            <div className="flex items-center gap-2">
              <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px]">v1.0</Badge>
              <Badge className="bg-[#0081C9]/20 text-[#0081C9] border-[#0081C9]/30 text-[10px]">REST</Badge>
            </div>
          </div>
          <div className="divide-y divide-slate-100 max-h-[500px] overflow-y-auto">
            {filteredEndpoints.map((endpoint, i) => (
              <div
                key={i}
                className={`flex items-center justify-between px-4 py-3 hover:bg-slate-50 cursor-pointer transition ${
                  selectedEndpoint?.path === endpoint.path ? "bg-blue-50 border-l-2 border-[#0081C9]" : ""
                }`}
                onClick={() => setSelectedEndpoint(endpoint)}
                data-testid={`api-endpoint-${i}`}
              >
                <div className="flex items-center gap-3">
                  <Badge className={`font-mono text-[10px] w-12 justify-center ${
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge className={`text-[10px] border-none ${
                    endpoint.status === "live" ? "bg-emerald-100 text-emerald-700" :
                    endpoint.status === "beta" ? "bg-amber-100 text-amber-700" :
                    "bg-slate-100 text-slate-500"
                  }`}>
                    {endpoint.status}
                  </Badge>
          ...
          <Card className="mt-4 border-none shadow-sm bg-slate-900 text-white">
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Badge className={`font-mono text-xs ${selectedEndpoint.method === "GET" ? "bg-emerald-500" : "bg-blue-500"} text-white border-none`}>
                    {selectedEndpoint.method}
                  </Badge>
                  <code className="text-sm text-[#0081C9]">{selectedEndpoint.path}</code>
                </div>
                <Button size="sm" className="bg-[#0081C9] hover:bg-blue-700 text-white h-7 text-xs" data-testid="button-try-api">
    ...
    "example": "data"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="bg-gradient-to-br from-[#0081C9]/5 to-[#0081C9]/10 border-[#0081C9]/20">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-slate-600 mb-4">
              Get your API key and start building with the One Flow API. Full documentation, SDKs, and sandbox environment included.
            </p>
            <Button className="bg-[#0081C9] hover:bg-blue-700 text-white" data-testid="button-get-api-key">
              <Code className="h-4 w-4 mr-2" /> Get API Key
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-[#C1121F]/5 to-[#C1121F]/10 border-[#C1121F]/20">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-slate-600 mb-4">
              Become an OGA-verified partner. Connect your tourism business to the Global Trust Network and access cross-border loyalty rewards.
            </p>
            <Button className="bg-[#C1121F] hover:bg-red-800 text-white" data-testid="button-join-network">
              <Users className="h-4 w-4 mr-2" /> Apply as Partner
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="text-center py-4">
        <p className="text-xs text-slate-400">
          OnlineGuide.io &mdash; Online Guide Association (OGA) &mdash; <span className="font-semibold text-slate-500">Connecting Cultures with Loyalty and Truth</span>
        </p>
      </div>
    </div>
  );
